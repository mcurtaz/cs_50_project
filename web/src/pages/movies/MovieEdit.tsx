...
const MovieEdit: React.FC = () => {
  ...
  const movie = useLoaderData() as MovieResponse;
  ...
  return (
    <div className="w-full h-full flex flex-col p-6">
      <div className="flex items-center">
        ...
        <h1 className="flex-grow text-center text-3xl font-semibold">EDIT MOVIE</h1>
      </div>
      <MovieForm 
        movie={movie} 
        options={{
          redirect: "/movie", 
          success_message: "Movie edited successfully", 
          button_label : "Edit movie",
          method: "put"
        }}
      />
    </div> 
...
export const fetchMovie = async ({params}: {params: Params}) =>{
    ...
    try {
        let response = await axios.get<MovieResponse>(
...
export const submitMovieAction = async ({request, params}: {request: Request, params: Params}) => {
  let is_token_valid = await verifyToken();
